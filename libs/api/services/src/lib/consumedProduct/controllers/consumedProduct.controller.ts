...
import type { ConsumedProduct } from '../models'
...
export class ConsumedProductsController {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('meal/:userId')
  async findAllByMeal(@Param('userId') userId: number,@Body() params: Pick<ConsumedProduct, 'meal'>) {

    return this.consumedProductsService.findAllByMeal(params.meal, userId)
  }
}
